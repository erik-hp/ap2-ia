"""Gradient checking por diferenças finitas para a MLP NumPy."""
...
def gradient_check(model: NumpyMLP, x: np.ndarray, y: np.ndarray, eps: float = 1e-5, tolerance: float = 1e-5) -> float:
    """Compara gradientes analíticos e numéricos por diferenças centrais."""
    if eps <= 0:
    ...
    # Cada peso e bias é perturbado individualmente para comparar o gradiente
    # analítico do backprop com o gradiente numérico.
    ...
    # O limite 1e-5 é o critério exigido no enunciado.
