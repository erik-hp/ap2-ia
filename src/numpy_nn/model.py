"""MLP configurável com forward/backward explícitos em NumPy."""
...
class NumpyMLP:
    """MLP totalmente implementada com NumPy e backpropagation explícito.

    :param layer_sizes: Quantidade de neurônios da entrada até a saída.
    :param seed: Seed usada na inicialização He.
    :raises ValueError: Se a arquitetura for inválida.
    """
    ...
    def __init__(self, layer_sizes: list[int], seed: int = 42):
        ...
        # Inicialização He: boa escolha para camadas com ReLU, pois preserva
        # melhor a escala dos sinais durante a propagação.
    ...
    def _prepare_input(self, x: np.ndarray) -> np.ndarray:
        ...
        RGB em ``N x H x W x 3`` ou ``N x 3 x H x W``. A função não
    ...
    def forward(self, x: np.ndarray) -> tuple[np.ndarray, list[dict[str, np.ndarray]]]:
        ...
        for i in range(1, n_layers + 1):
            ...
            # As camadas escondidas usam ReLU; a última camada retorna logits
            # crus, pois o softmax é aplicado dentro da loss numericamente estável.
            if i < n_layers:
                a = np.maximum(0.0, z)
            else:
                a = z
    ...
    def backward(self, d_logits: np.ndarray, caches: list[dict[str, np.ndarray]]) -> dict[str, np.ndarray]:
        """Calcula gradientes dos parâmetros a partir do gradiente dos logits."""
        grads: dict[str, np.ndarray] = {}
        da = d_logits

        # Backpropagation da última camada até a primeira, aplicando a regra da
